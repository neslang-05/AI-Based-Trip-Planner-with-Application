...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        leaf/.style={draw, rounded corners, minimum width=2.8cm,
                     minimum height=0.7cm, align=center, font=\small, fill=green!15},
        arrow/.style={-Stealth, thick}
    ]
        % Root node
        \node[decision, fill=blue!15] (root) at (0, 0) {Budget $\leq$ threshold?};

        % Level-2 nodes — wider horizontal spread to avoid overlap
        \node[decision, fill=blue!15] (left)  at (-4.5, -2.5) {Climate = Cold?};
        \node[decision, fill=blue!15] (right) at ( 4.5, -2.5) {Travel Type?};

        % Leaf nodes — well-separated
        \node[leaf] (ll) at (-6.5, -5) {Shimla};
        \node[leaf] (lr) at (-2.5, -5) {Rishikesh};
        \node[leaf] (rl) at ( 2.5, -5) {Goa};
        \node[leaf] (rr) at ( 6.5, -5) {Manali};

        % Root to branches
        \draw[arrow] (root.south west) -- node[left,  font=\scriptsize]{Yes} (left.north);
        \draw[arrow] (root.south east) -- node[right, font=\scriptsize]{No}  (right.north);

        % Left branch to leaves
        \draw[arrow] (left.south west) -- node[left,  font=\scriptsize]{Yes} (ll.north);
        \draw[arrow] (left.south east) -- node[right, font=\scriptsize]{No}  (lr.north);

        % Right branch to leaves
        \draw[arrow] (right.south west) -- node[left,  font=\scriptsize]{Relax} (rl.north);
        \draw[arrow] (right.south east) -- node[right, font=\scriptsize]{Adv.}  (rr.north);
    \end{tikzpicture}

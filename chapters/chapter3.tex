...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=1.2cm,
        box/.style={draw, rounded corners, minimum width=5cm, minimum height=0.9cm, align=center, font=\small},
        arrow/.style={-Stealth, thick}
    ]
        \node[box, fill=blue!15] (user) {User (Web Browser)};
        \node[box, fill=green!15, below=of user] (frontend) {Frontend Layer (HTML5, CSS3, JavaScript)};
        \node[box, fill=orange!15, below=of frontend] (django) {Django Web Application (Views, URLs, Templates)};
        \node[box, fill=purple!15, below=of django] (api) {Django REST Framework (API Layer)};
        \node[box, fill=red!15, below=of api] (ml) {Machine Learning Engine (Decision Tree, Joblib)};
        \node[box, fill=gray!15, below=of ml] (db) {Database (SQLite3 / ORM)};
        ...
        \draw[arrow] (api) -- (ml);
        \draw[arrow] (ml) -- (db);
        \draw[arrow, dashed] (db.east) -- ++(1.5,0) |- (user.east);
    \end{tikzpicture}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=0.9cm,
        step/.style={draw, rounded corners, minimum width=7cm, minimum height=0.8cm, align=center, font=\small},
        arrow/.style={-Stealth, thick}
    ]
        \node[step, fill=blue!10] (s1) {1. User visits the application and registers/logs in};
        \node[step, fill=blue!10, below=of s1] (s2) {2. User fills the Trip Planning Form (budget, days, climate, travel type)};
        \node[step, fill=green!10, below=of s2] (s3) {3. Form data submitted via POST request to Django API};
        \node[step, fill=green!10, below=of s3] (s4) {4. API serializer validates input data};
        \node[step, fill=orange!10, below=of s4] (s5) {5. ML engine predicts destination using Decision Tree};
        \node[step, fill=orange!10, below=of s5] (s6) {6. Itinerary generator creates day-wise schedule and cost breakdown};
        \node[step, fill=purple!10, below=of s6] (s7) {7. Results saved to database and returned as JSON response};
        \node[step, fill=purple!10, below=of s7] (s8) {8. Frontend renders Results View with full trip plan};
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        entity/.style={draw, rectangle, minimum width=3cm, minimum height=0.8cm, align=center, font=\small\bfseries},
        attr/.style={draw, ellipse, minimum width=2.2cm, minimum height=0.6cm, align=center, font=\small},
        arrow/.style={-Stealth, thick}
    ]
        % USER entity
        \node[entity, fill=blue!20] (user) at (0,0) {USER};
        \node[attr] (uid) at (-2.5, 1.2) {id (PK)};
        \node[attr] (uname) at (0, 1.5) {username};
        \node[attr] (uemail) at (2.5, 1.2) {email};
        \node[attr] (upass) at (-2.5, -1.2) {password};

        % TRIP entity
        \node[entity, fill=green!20] (trip) at (7,0) {TRIP};
        \node[attr] (tid) at (5, 1.5) {id (PK)};
        \node[attr] (tdest) at (7.5, 1.5) {destination};
        \node[attr] (tbudget) at (9.5, 1) {budget};
        \node[attr] (tdays) at (9.5, -1) {days};
        \node[attr] (tclimate) at (7.5, -1.5) {climate};
        \node[attr] (ttype) at (5, -1.5) {travel\_type};
        \node[attr] (titi) at (3.5, 0) {itinerary};
        ...
        % Relationship
        \draw[thick] (user.east) -- node[above, font=\small]{1..N} (trip.west);
    \end{tikzpicture}
